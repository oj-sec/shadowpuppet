...
class Embedder():
    ...
    def __init__(
        ...
        try:
            ...
            if "couldn't find them in the cached files" in str(e):
                logging.info("Embedder could not file model %s locally.", self.model_string)
            else:
                logging.critical("Embedder failed to check model. Exception: %s, %s", type(e).__name__, str(e))
                raise e
    ...
    def iterate_database(self, database_filename):
        ...
        storage_field = database_connector.create_field_to_store_embeddings(self.embedding_field)

        while True:
            rows = database_connector.get_unenriched_documents(storage_field, self.max_batch_size)
            if not rows:
