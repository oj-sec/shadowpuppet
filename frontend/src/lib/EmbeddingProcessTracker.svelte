<script lang="ts">
    ...
    let percentCompleted = $derived(
        Math.floor((completedDocuments / totalDocuments) * 100),
    );
    let documentsPerMinute = $state(0);
    let estimatedSecondsRemaining = $state(0);
    let estimatedParsedTimeRemaining = $derived(
    ...
    $effect(() => {
        if (completedDocuments > 0 && totalDocuments > 0) {
            documentsPerMinute = Math.floor(
                (completedDocuments / (Date.now() - startTime)) * 60 * 1000,
            );
            estimatedSecondsRemaining = Math.floor(
                (remainingDocuments / documentsPerMinute) * 60,
            );
        }
    });
    ...
    async function startEmbedding() {
        ...
        statusMessage = "Embedding documents...";
    ...
    async function progressWatcherFunction() {
        setInterval(async () => {
            ...
            if (
                ...
                !responseJson.nearNeighbourComplete
            ) {
                statusMessage = "Computing neighbours";
            }
...
<div class="m-4">
    <Progress value={completedDocuments} max={totalDocuments}
        >{percentCompleted}%</Progress
    >
</div>
<div
    class="text-center italic opacity-60 h-6 flex items-center justify-center mb-2"
>
    {statusMessage || ""}
</div>
<div>
...
<button
    disabled={embeddingStarted}
    onclick={startEmbedding}
    type="button"
    class="btn mt-2 preset-filled"
>
    <span>Start</span>
</button>
